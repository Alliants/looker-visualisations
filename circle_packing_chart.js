looker.plugins.visualizations.add({
    ...
    updateDynamicOptions: function(queryResponse) {
      ...
      Object.keys(this.options).forEach((key) => {
        if (key.startsWith("metric_label_") || key.startsWith("metric_icon_") || key.startsWith("metric_color_")) {
    ...
    updateAsync: function(data, element, config, queryResponse, details, done) {
      ...
      const metrics = fields.map((field, index) => {
        ...
        const color = config[`metric_color_${index}`] || "#000000";
        const icon = config[`metric_icon_${index}`] || "";
        return {
          label: label,
          value: isNaN(value) ? 0 : Number(value),
          color: color,
          icon: icon + "&color=" + that.hexToRgb(color) + ",1"
        };
      ...
      for (let i = 1; i < metrics.length; i++) {
        ...
        const calloutContent = `
          <div class="metric-block">
            <div class="small-circle" style="width: ${sizePercentage}vw; height: ${sizePercentage}vw; font-size: ${fontSizePercentage}vw; background-color: ${config.small_circle_color}; color: ${metrics[i].color};">
              ${metrics[i].value}
            </div>
